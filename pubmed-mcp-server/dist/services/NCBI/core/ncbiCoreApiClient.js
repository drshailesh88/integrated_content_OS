        ...
        for (let attempt = 0; attempt <= config.ncbiMaxRetries; attempt++) {
            try {
                ...
                if (error instanceof McpError)
                    throw error;
                logger.error(`Unexpected error during NCBI request to ${endpoint} after ${attempt} retries`, err, requestContextService.createRequestContext({
                    ...context,
                    operation: "NCBI_UnexpectedError",
                    endpoint,
                    errorMessage: err.message,
                }));
                throw new McpError(BaseErrorCode.INTERNAL_ERROR, `Unexpected error communicating with NCBI: ${err.message}`, { endpoint });
            }
        }
...
//# sourceMappingURL=ncbiCoreApiClient.js.map