...
export class StatefulTransportManager extends BaseTransportManager {
     ...
     */
    constructor(createServerInstanceFn, options) {
        ...
        this.garbageCollector = setInterval(() => this.cleanupStaleSessions(), this.options.staleSessionTimeoutMs);
    }
    /**
    ...
    async initializeAndHandle(headers, body, context) {
        ...
        try {
            ...
            logger.error("Failed to initialize stateful session. Cleaning up orphaned resources.", error instanceof Error ? error : undefined, { ...opContext, error: String(error) });
            const sessionInitialized = transport?.sessionId && this.transports.has(transport.sessionId);
    ...
    async handleRequest(headers, body, context, sessionId) {
        ...
        logger.debug(`Incremented activeRequests for session ${sessionId}. Count: ${session.activeRequests}`, sessionContext);
        try {
            const mockReq = {
                headers,
                method: "POST",
            ...
            session.activeRequests -= 1;
            session.lastAccessedAt = new Date();
            logger.debug(`Decremented activeRequests for session ${sessionId}. Count: ${session.activeRequests}`, sessionContext);
        }
    }
    /**
    ...
    async cleanupStaleSessions() {
        ...
        for (const [sessionId, session] of this.sessions.entries()) {
            if (now - session.lastAccessedAt.getTime() > STALE_TIMEOUT_MS) {
                if (session.activeRequests > 0) {
                    logger.info(`Session ${sessionId} is stale but has ${session.activeRequests} active requests. Skipping cleanup.`, { ...context, sessionId });
                    continue;
                }
        ...
        if (staleSessionIds.length > 0) {
            logger.info(`Found ${staleSessionIds.length} stale sessions. Closing concurrently.`, context);
            const closePromises = staleSessionIds.map((sessionId) => this.closeSession(sessionId, context).catch((err) => {
                logger.error(`Error during concurrent stale session cleanup for ${sessionId}`, err, { ...context, sessionId });
            }));
            await Promise.all(closePromises);
            logger.info(`Stale session cleanup complete. Closed ${staleSessionIds.length} sessions.`, context);
        }
        else {
...
//# sourceMappingURL=statefulTransportManager.js.map