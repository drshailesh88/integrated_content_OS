...
export const PubMedSearchArticlesInputSchema = z.object({
    ...
    sortBy: z
        ...
        .optional()
        .default("relevance")
        .describe("Sorting criteria for results. Options: 'relevance' (default), 'pub_date', 'author', 'journal_name'."),
    dateRange: z
        .object({
        minDate: z
            .string()
    ...
    filterByPublicationTypes: z
        .array(z.string())
        .optional()
        .describe('An array of publication types to filter by (e.g., ["Review", "Clinical Trial"]).'),
    fetchBriefSummaries: z
        ...
        .optional()
        .default(0)
        .describe("Number of top PMIDs for which to fetch brief summaries using ESummary. Set to 0 to disable. Max 50. Default 0."),
});
export async function pubmedSearchArticlesLogic(input, parentRequestContext) {
    ...
    if (currentFetchBriefSummaries > 0 && pmids.length > 0) {
        ...
        else if (eSummaryResponseXml &&
            eSummaryResponseXml.eSummaryResult?.ERROR) {
            logger.warning("ESummary returned a top-level error", {
                ...toolLogicContext,
                errorDetails: eSummaryResponseXml
